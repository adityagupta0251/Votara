...
function DashboardView({ onSelectProposal }: { onSelectProposal: (p: ProposalWithPubkey) => void }) {
    return (
        <div className="space-y-12">
            <SEO title="Institutional Dashboard" />
            <SectionHeader
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-8">
                    <section>
                        ...
                        <AllProposals onSelect={onSelectProposal} />
                    </section>
                </div>
...
function ProposalsView({ onSelectProposal }: { onSelectProposal: (p: ProposalWithPubkey) => void }) {
    return (
        <div className="space-y-10">
            <SEO title="Governance Proposals" />
            <SectionHeader
                title="Governance Stream"
                subtitle="Review and participate in active community proposals."
            />
            <AllProposals onSelect={onSelectProposal} />
        </div>
    );
...
function CreateProposalView({ nextId }: { nextId: number }) {
    return (
        <div className="max-w-4xl space-y-10">
            <SEO title="Create New Proposal" />
            <SectionHeader
...
function TreasuryView({ userTokenAccount }: { userTokenAccount: any }) {
    return (
        <div className="max-w-4xl space-y-10">
            <SEO title="Treasury Management" />
            <SectionHeader
...
function ProfileView() {
    return (
        <div className="max-w-2xl space-y-10">
            <SEO title="Voter Profile" />
            <SectionHeader
                title="Voter Profile"
                subtitle="Manage your identity and participation metrics."
            />
            <VoterInfo />
            <RegisterVoter />
        </div>
                        ...
                        {selectedProposal ? (
                            ...
                            >
                                <Routes location={location}>
                                    ...
                                    <Route path="/dashboard" element={<DashboardView onSelectProposal={setSelectedProposal} />} />
                                    <Route path="/proposals" element={<ProposalsView onSelectProposal={setSelectedProposal} />} />
